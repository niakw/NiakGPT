...
- **Débit amélioré** : le mode prioritaire groupe jusqu’à 3 conversations réussies — sous plafonds de fichiers/octets — dans un même checkpoint GitHub. Le ref/commit de branche reste sérialisé et non forcé.
...
- **Non-régression** : `project-memory-transient-fetch-v117.mjs` injecte successivement un 500 puis `Failed to fetch` sur un chat, exige que les chats suivants soient archivés et batchés, vérifie l’absence de boucle immédiate, puis libère le retry et exige une convergence sans refetch des chats déjà importés.

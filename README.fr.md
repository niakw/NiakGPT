...
> **Version actuelle : 0.9.109.** L’hydratation React est vérifiée dans le **monde MAIN**. NiakGPT peut désormais retrouver le HostRoot soit via le conteneur React, soit en remontant la chaîne réelle `__reactFiber$…` depuis les nœuds du document. Il exige toujours un HostRoot stabilisé, l’ownership React explicite de `<html>` + `<body>` et une confirmation des nœuds hôtes avant toute mutation DOM/CSS.

## Points forts

### Démarrage protégé contre les erreurs d’hydratation

La 0.9.109 conserve la frontière zéro-touch et ajoute une seconde voie d’autorité React : **aucun CSS NiakGPT n’est déclaré comme content script statique**. Les 34 feuilles de style sont injectées uniquement après validation du gate d’hydratation, juste avant le runtime. Cela évite qu’un style NiakGPT puisse influencer le rendu client de ChatGPT pendant l’hydratation SSR.
...
| [RELEASE_NOTES_0.9.109.md](RELEASE_NOTES_0.9.109.md) | Résumé de la release courante |

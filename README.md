...
> **Current version: 0.9.118.** Project Memory now treats the Project index as a monotonic union instead of a last-writer snapshot. If an older tab/runtime overwrites the index with only a few rows, NiakGPT rebuilds it from the per-conversation checkpoints already present in the private vault before fetching history again. Existing archived conversations are recognized and skipped rather than replayed from 0%.

...
> **Current version: 0.9.107.** React hydration is verified from the page’s **MAIN world** through the MV3 service worker. NiakGPT no longer treats volatile `data-build` / React Router globals as proof that a page is safe: DOM and CSS remain untouched until a real React container is found and the HostRoot is settled.

## Highlights

### Hydration-safe startup

0.9.107 keeps the zero-touch CSS boundary introduced in 0.9.105 and removes the last heuristic hydration bypass: **no NiakGPT CSS is declared as a static content script**. All 34 stylesheets are inserted only after the hydration gate succeeds, immediately before the runtime scripts. This prevents early layout/style influence from changing ChatGPT's client render during SSR hydration.

(() => {
  ...
  const busy=()=>document.hidden||document.documentElement.dataset.ng90Safe==='1'||document.documentElement.dataset.ng8Running==='1'||['loading','waiting','thinking','executing'].includes(String(document.documentElement.dataset.ng86Activity||'').toLowerCase())||document.documentElement.dataset.ng105Verification==='1'||['verify','network'].includes(String(document.documentElement.dataset.ng119Interruption||'').toLowerCase())||Number(document.documentElement.dataset.ng100RateLimitedUntil||0)>Date.now()||Number(document.documentElement.dataset.ng100NativePriorityUntil||0)>Date.now();

(() => {
  ...
  let value=null,loaded=false,contextDead=false,writeChain=Promise.resolve();
  ...
  async function storageSet(next){
    ...
    catch(error){if(invalidated(error)){markDead();return false;}console.warn('[NiakGPT cache bus write]',error);return false;}
  }
  ...
  const api={
    ...
    subscribe(fn){
      if(typeof fn!=='function'||contextDead)return()=>{};
      listeners.add(fn);

(() => {
  ...
  let state={schema:1,initialized:false,seen:{}},cache={projects:[],chats:[]},timer=0,persistTimer=0,responseActivity=false,touching=false,contextDead=false;
  ...
  function markDead(){
    if(contextDead)return;contextDead=true;clearTimeout(timer);clearTimeout(persistTimer);timer=persistTimer=0;
    try{document.documentElement.dataset.ng113AttentionContext='inactive';}catch{}
    try{window.__NIAKGPT_DIAGNOSTICS__?.set('chat-attention','INACTIF · contexte extension invalidé');}catch{}
  }
  ...
  function apply(){
    timer=0;if(contextDead)return;initializeIfNeeded();const current=currentCid();if(current&&!document.hidden)markSeen(current);
  ...
  function schedule(delay=25){if(contextDead)return;clearTimeout(timer);timer=setTimeout(apply,delay);}
  function accept(raw){if(contextDead)return;if(raw&&typeof raw==='object')cache=raw;initializeIfNeeded();schedule(15);}
  async function start(){
    if(!contextAlive()){markDead();return;}
    try{const got=await chrome.storage.local.get([STATE_KEY,CACHE_KEY]);state=got[STATE_KEY]&&typeof got[STATE_KEY]==='object'?got[STATE_KEY]:state;state.seen=state.seen||{};cache=got[CACHE_KEY]||cache;}catch(error){if(invalidated(error)||!contextAlive()){markDead();return;}}
  ...
  try{chrome.storage.onChanged.addListener((changes,area)=>{if(contextDead)return;if(area==='local'&&changes[CACHE_KEY])accept(changes[CACHE_KEY].newValue);});}catch(error){if(invalidated(error)||!contextAlive())markDead();}
  start();
})();

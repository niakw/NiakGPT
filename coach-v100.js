(() => {
  ...
  const enabled=()=>root.dataset.ng90Coach!=='off'&&root.dataset.ng90Safe!=='1'&&!['waiting','thinking','executing'].includes(root.dataset.ng86Activity||'');
  ...
  function classify(prompt){
    const p=norm(prompt),ctx=norm(recent());
    const scores={perf:0,design:0,code:0,research:0,organize:0,legal:0,data:0,writing:0,general:1};
    const add=(key,rx,weight)=>{if(rx.test(p))scores[key]+=weight;else if(rx.test(ctx))scores[key]+=1;};
    add('perf',/perf|performance|lent|lourd|lag|cache|cpu|mémoire|memoire|réseau|reseau|optim|reflow|polling|timer|onglet/,12);
    add('design',/design|\bda\b|interface|\bui\b|\bux\b|visuel|couleur|layout|thème|theme|responsive|sidebar|barre|panel|panneau|chevauch|décalage|decalage/,14);

(() => {
  ...
  function commandList(){return[
    {id:'quick',icon:'⌘',title:'Ouverture rapide',hint:'Alt+K',keys:'quick ouverture chats conversations projets projects recherche',run:()=>{close();click('#ng8-rail [data-q],#ng8-status [data-q]');}},
    {id:'settings',icon:'⚙',title:'Centre de contrôle',hint:'Alt+,',keys:'settings paramètres preferences configuration centre contrôle',run:()=>{close();click('#ng90-settings-btn');}},
    {id:'safe',icon:'◈',title:'Basculer le Mode sûr',hint:'Performance',keys:'safe mode sûr performance lourd secours',run:async()=>{const on=document.documentElement.dataset.ng90Safe==='1';close();await patchSettings({safeMode:!on});}},
    {id:'explorer',icon:'▤',title:'Ouvrir l’explorateur',hint:'Projets',keys:'explorer explorateur projets projects',run:()=>{close();openTab('explorer');}},
    ...
    {id:'governance',icon:'§',title:'Gouvernance des projets',hint:'Nettoyer & reconstruire',keys:'governance gouvernance projets tri classement nettoyage',run:()=>{close();openTab('explorer');setTimeout(()=>click('#ng8-panel [data-repair]'),120);}},
    ...
    {id:'power',icon:'◆',title:'Profil : Complet',hint:'Espace de travail',keys:'profile profil power complet',run:()=>setProfile('power')},
    {id:'code',icon:'</>',title:'Profil : Code / IDE',hint:'Espace de travail',keys:'profile profil code ide dev',run:()=>setProfile('code')},
    {id:'research',icon:'▧',title:'Profil : Recherche',hint:'Espace de travail',keys:'profile profil research recherche lecture',run:()=>setProfile('research')},
    {id:'focus',icon:'○',title:'Profil : Focus / Rédaction',hint:'Espace de travail',keys:'profile profil focus writing rédaction ecriture calme',run:()=>setProfile('focus')},
    {id:'analyst',icon:'▦',title:'Profil : Analyse',hint:'Espace de travail',keys:'profile profil analyst analyse data tableau',run:()=>setProfile('analyst')},
    {id:'contrast',icon:'◐',title:'Profil : Contraste élevé',hint:'Accessibilité',keys:'profile profil contrast contraste accessibility accessibilite',run:()=>setProfile('contrast')}
  ];}

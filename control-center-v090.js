(() => {
  ...
  const VERSION = (() => { try { return chrome.runtime.getManifest().version || '0.9.2'; } catch { return '0.9.2'; } })();
  ...
  function decorateStatus() {
    const status=document.getElementById('ng8-status');if(!status)return;let badge=status.querySelector('.ng90-safe-badge');if(settings.safeMode){if(!badge){badge=document.createElement('span');badge.className='ng90-safe-badge';badge.textContent='SAFE';status.appendChild(badge);}}else badge?.remove();
  }
  function ensureButton() {
    clearTimeout(ensureTimer);const rail=document.getElementById('ng8-rail');if(!rail){ensureTimer=setTimeout(ensureButton,700);return;}
    if(!rail.querySelector('#ng90-settings-btn')){const button=document.createElement('button');button.id='ng90-settings-btn';button.type='button';button.setAttribute('aria-label','Ouvrir le Centre de contrôle NiakGPT');button.title='Centre de contrôle · Alt+,';button.textContent='⚙';const spacer=rail.querySelector(':scope > span');rail.insertBefore(button,spacer||null);button.addEventListener('click',openModal);}decorateStatus();
  }
  ...
  function pageKind(){const p=location.pathname;if(/\/c\//.test(p))return'conversation';if(/\/g\/g-p-[^/]+\/project/.test(p))return'project';return'home';}
  ...
  async function rebuildIndex(){if(!confirm('Effacer uniquement l’index Projects/chats de NiakGPT et le reconstruire au prochain chargement ?'))return;try{await chrome.storage.local.remove(CACHE_KEY);}catch{}toast('Index supprimé · rechargement…');setTimeout(()=>location.reload(),500);}
  async function resetPreferences(){if(!confirm('Réinitialiser uniquement les préférences visuelles et de performance ?\n\nLa structure Projects et les verrous manuels seront conservés.'))return;settings={...DEFAULTS};try{await chrome.storage.local.remove(SETTINGS_KEY);}catch{}await saveSettings(settings);toast('Préférences réinitialisées');}
  ...
  function renderModal() {
    ...
    modal.innerHTML=`<div class="ng90-card">
      <header><div><small>CONTROL CENTER · ${esc(VERSION)}</small><b id="ng90-control-title">NiakGPT</b><span>Workspace power-user local pour ChatGPT</span></div><button data-close aria-label="Fermer le Centre de contrôle">×</button></header>
      <div class="ng90-safe ${settings.safeMode?'on':''}"><div><b>SAFE MODE</b><span>${settings.safeMode?'Actif — animations et automatisations suspendues':'Mode de secours ultra-léger pour les fils extrêmes'}</span></div><label><input type="checkbox" data-setting="safeMode" ${settings.safeMode?'checked':''}><i></i></label></div>
      <div class="ng90-grid">
        <section><h3>APPARENCE</h3><label class="ng90-select"><span><b>Matrix</b><small>Intensité du fond animé</small></span><select data-setting="matrix"><option value="normal" ${settings.matrix==='normal'?'selected':''}>Normal</option><option value="subtle" ${settings.matrix==='subtle'?'selected':''}>Subtil</option><option value="off" ${settings.matrix==='off'?'selected':''}>Off</option></select></label><label class="ng90-select"><span><b>Densité</b><small>Sidebar et métadonnées</small></span><select data-setting="density"><option value="compact" ${settings.density==='compact'?'selected':''}>Compacte</option><option value="comfortable" ${settings.density==='comfortable'?'selected':''}>Confortable</option></select></label>${toggleRow('motion','Animations','Pulse d’activité et transitions')}${toggleRow('activityColors','Couleurs d’activité','Chats / Projects / barre basse')}${toggleRow('easterEggs','Easter eggs','Robots et clins d’œil SKYNET')}</section>
        <section><h3>ASSISTANCE</h3>${toggleRow('coach','Coach de prompts','Suggestions contextuelles dans le flux du composer')}${toggleRow('dates','Dates des chats','Dernier échange dans la sidebar')}${toggleRow('projectBadges','Badges Projects','Projet associé à chaque conversation')}${toggleRow('statusBar','Barre d’état','État, Project et BY SKYNET')}</section>
        <section><h3>AUTOMATISATION</h3>${toggleRow('autoResync','Resynchronisation automatique','Classe uniquement les chats hors projet non verrouillés')}${toggleRow('nativePins','Pins natifs','Synchronise les Projects principaux dans ChatGPT')}<div class="ng90-info"><b>Manuel &gt; automatique</b><span>Un chat déplacé manuellement reste verrouillé et n’est jamais reclassé sans action explicite.</span></div></section>
        <section><h3>PERFORMANCE & CACHE</h3><div class="ng90-metrics"><div><b>${h.entries}</b><span>fils chauds</span></div><div><b>${h.hits}</b><span>hits</span></div><div><b>${h.network}</b><span>réseau</span></div><div><b>${h.mb.toFixed(1)}</b><span>Mo</span></div></div><div class="ng90-actions"><button data-purge-cache>Purger cache chaud</button><button data-rebuild-index>Reconstruire l’index</button><button data-copy-diag>Copier diagnostic</button></div><div class="ng90-info"><b>Local-first</b><span>Aucun serveur NiakGPT, aucune analytics, aucune API payante.</span></div></section>
      </div>
  ...
  readMirror();applySettings();loadSettings();ensureButton();
})();

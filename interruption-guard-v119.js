(() => {
  ...
  const KEY='niakgpt-interruption-v119',DRAFT_PREFIX='niakgpt-draft-v119:';
  ...
  let observer=null,timer=0,incident=readIncident(),marking=false,lastAssistantLen=0,lastGrowthAt=0;
  ...
  function editorText(ed=editor()){return clean(ed&&('value'in ed?ed.value:ed.innerText||ed.textContent));}
  function saveDraft(){const ed=editor();if(!ed)return;try{const text='value'in ed?ed.value:ed.innerText||ed.textContent;text?sessionStorage.setItem(draftKey(),text):sessionStorage.removeItem(draftKey());}catch{}}
  function restoreDraft(){const ed=editor();if(!ed||editorText(ed))return false;let text='';try{text=sessionStorage.getItem(draftKey())||'';}catch{}return !!text&&setEditor(ed,text);}
  ...
  function publishVerification(on){
    ...
    if(on){root.dataset.ng105Verification='1';root.dataset.ng119Verification='1';return;}
    if(root.dataset.ng119Verification==='1'){delete root.dataset.ng105Verification;delete root.dataset.ng119Verification;}
  }
  ...
  function currentSignal(type=''){
    const nodes=[...document.querySelectorAll('[role="alert"],[role="status"],[data-testid*="error" i],[data-testid*="limit" i],[data-testid*="toast" i],[data-testid*="challenge" i],iframe[src*="challenge" i],iframe[src*="cloudflare" i]')].filter(el=>visible(el)&&!ownRecoveryNode(el));
  ...
  function resumePrompt(){
    ...
    const ed=editor();if(!ed||editorText(ed))return false;return setEditor(ed,text);
  }
  function mount(type,{ready=false}={}){
    ...
    const msg=type==='verify'?'NiakGPT suspend son trafic et reprend après la vérification native.':'NiakGPT conserve le brouillon et utilise le bouton natif de reprise lorsque la connexion revient.';
    box.innerHTML=`<strong>${label}</strong><span>${msg}</span><button type="button" data-ng119-resume>${ready?'REPRENDRE':'EN ATTENTE'}</button>`;
    const b=box.querySelector('[data-ng119-resume]');b.disabled=!ready;b.addEventListener('click',()=>{if(resumePrompt()){box.querySelector('span').textContent='Brouillon ou message de reprise restauré dans le champ. Aucun envoi automatique.';b.disabled=true;b.textContent='PRÊT À ENVOYER';}else{box.querySelector('span').textContent='Le champ contient déjà du texte ou n’est pas disponible : rien n’a été écrasé.';}});return box;
  }
  async function handleLimit(reason='dom-limit'){
    const cid=String(location.pathname).match(/\/c\/([A-Za-z0-9_-]+)/)?.[1]||'';if(!cid)return;
    saveDraft();if(!incident||incident.type!=='limit'||incident.chatId!==cid)saveIncident({type:'limit',chatId:cid,at:Date.now(),reason,retried:false});
  ...
  function begin(type,text=''){
    ...
    saveDraft();if(type==='verify')publishVerification(true);else if(incident?.type==='verify'&&!currentSignal('verify'))publishVerification(false);
    if(!incident||incident.type!==type)saveIncident({type,at:Date.now(),retried:false,recovered:false,sample:clean(text).slice(0,240)});
    mount(type,{ready:false});window.__NIAKGPT_DIAGNOSTICS__?.set('interruption-119',type==='verify'?'PAUSE · vérification native · trafic NiakGPT suspendu':'PAUSE · connexion · brouillon conservé');scheduleRecovery(500);
  }
  function recoveryStable(){
    const len=assistantLength();if(len>lastAssistantLen){lastAssistantLen=len;lastGrowthAt=Date.now();scheduleRecovery(700);return false;}
    if(nativeBusy()||Date.now()-lastGrowthAt<1800){scheduleRecovery(700);return false;}return true;
  }
  ...
  function inspectNode(node){
    ...
    if(!type){for(const el of node.querySelectorAll?.('[role="alert"],[role="status"],[data-testid*="error" i],[data-testid*="limit" i],[data-testid*="toast" i],[data-testid*="challenge" i]')||[]){if(ownRecoveryNode(el))continue;type=classifyText(candidateText(el));if(type)break;}}
    if(type)begin(type,text);
  }
  function scan(){const signal=currentSignal();if(signal)begin(signal.kind,candidateText(signal.node));else if(incident&&['verify','network'].includes(incident.type))scheduleRecovery(120);else publishVerification(false);}
  function bind(){
    observer?.disconnect();observer=new MutationObserver(records=>{
      ...
      if(structural){const len=assistantLength();if(len>lastAssistantLen){lastAssistantLen=len;lastGrowthAt=Date.now();}if(incident&&['verify','network'].includes(incident.type))scheduleRecovery(180);}
    });observer.observe(document.documentElement,{childList:true,subtree:true});
  }
  function onRoute(){clearBar();const p=String(location.pathname);if(!/\/c\//.test(p)&&incident?.type==='limit')saveIncident(null);setTimeout(()=>{restoreDraft();scan();},160);}
  ...
  document.addEventListener('input',event=>{const target=event.target instanceof Element?event.target:null;if(target&&(target===editor()||target.closest?.('[data-type="unified-composer"]')))saveDraft();},true);

...
if 'suppressNativeProjects' in legacy106 or "classList.add('ng8-native-project" in legacy106 or 'sidebarObserver' in legacy106 or 'attributes:true' in legacy106: fail('live-fixes-v106 still mutates/observes native Projects authority')
...
if errors:
    ...
    sys.exit(1)
print(f"STATIC_CURRENT_PASS version={manifest['version']} runtime={len(main)+len(isolated)} refs={len(refs)} profiles=2 cached-browsers=on left-sidebar=atomic-top-layer-session-clean single-projects-authority")

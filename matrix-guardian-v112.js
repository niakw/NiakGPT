(() => {
  ...
  const enabled=()=>document.documentElement.dataset.ng90Safe!=='1'&&document.documentElement.dataset.ng90Matrix!=='off'&&!reduce();
  ...
  function ensureFallback(){
    ...
    fallback=document.createElement('canvas');fallback.id='ng112-matrix-fallback';fallback.setAttribute('aria-hidden','true');(document.querySelector('main')||document.body).prepend(fallback);ctx=fallback.getContext('2d',{alpha:true});sizeFallback();drawFrame(false);

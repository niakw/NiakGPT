(() => {
  'use strict';
  if (location.hostname !== 'chatgpt.com' || window.__NIAKGPT_ONBOARDING_100__) return;
  window.__NIAKGPT_ONBOARDING_100__ = true;

  const KEY='niakgpt-onboarding-v100';
  const PROFILE_KEY='niakgpt-profile-v100';
  const VERSION=(()=>{try{return chrome.runtime.getManifest().version||'1.0.0';}catch{return'1.0.0';}})();
  ...
  function hasLegacyMirror(){try{for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i)||'';if(k.startsWith('niakgpt-')||k.startsWith('__niakgpt_'))return true;}}catch{}return false;}
  async function shouldShow(){
    try{
      const all=await chrome.storage.local.get(null);if(all[KEY])return false;
      const previous=Object.keys(all).some(k=>k!==KEY&&(k.startsWith('niakgpt-')||k.startsWith('__niakgpt_')))||hasLegacyMirror();
      if(previous){await chrome.storage.local.set({[KEY]:{status:'upgrade-skipped',version:VERSION,at:Date.now()}});return false;}
      return true;
    }catch{return false;}
  }
  ...
  function render(){
    ...
    modal.innerHTML=`<div class="ng100-onboard-card"><div class="ng100-onboard-progress">${pages.map((_,i)=>`<i class="${i<=step?'on':''}"></i>`).join('')}</div><main>${pages[step]}</main><footer>${step?'<button data-prev>Retour</button>':'<span></span>'}<em>${step+1} / ${pages.length}</em><button class="primary" data-next>${step===pages.length-1?'Commencer':'Continuer'}</button></footer></div>`;
  ...
  document.addEventListener('keydown',event=>{if(!modal)return;if(event.key==='Tab')trap(event);if(event.key==='Escape'){event.preventDefault();/* intentional: first-run onboarding requires an explicit finish */}} ,true);

  shouldShow().then(show=>{if(!show)return;const launch=()=>{if(document.body)open();else setTimeout(launch,200);};setTimeout(launch,1600);});
})();

(() => {
  ...
  let observer=null,boxNode=null,bootstrapObserver=null,timer=0,stopped=false,outState={out:{}},revealedCid='';
  const cid=h=>String(h||'').match(/\/c\/([A-Za-z0-9_-]+)/)?.[1]||'';
  const currentCid=()=>cid(location.pathname);
  function revealActive(link,chatId){
    if(!chatId||revealedCid===chatId||!(link instanceof HTMLElement))return;
    const list=link.closest('.ng96-folder-list');if(!list)return;
    const lr=list.getBoundingClientRect(),rr=link.getBoundingClientRect();
    if(rr.top<lr.top||rr.bottom>lr.bottom)link.scrollIntoView({block:'nearest',inline:'nearest'});
    revealedCid=chatId;
  }
  function decorateLink(link){
    ...
    if(active)requestAnimationFrame(()=>revealActive(link,chatId));
  }
  ...
  function stop(){stopped=true;clearTimeout(timer);observer?.disconnect();bootstrapObserver?.disconnect();observer=bootstrapObserver=null;boxNode=null;revealedCid='';}
  function onRoute(){revealedCid='';bind();schedule(0);}
  ...
  document.addEventListener('niakgpt:pins-rendered',()=>{bind();schedule(0);});document.addEventListener('niakgpt:folder-rendered',()=>schedule(0));document.addEventListener('visibilitychange',()=>{if(!document.hidden)onRoute();});window.addEventListener('popstate',onRoute);if(window.navigation?.addEventListener)window.navigation.addEventListener('navigatesuccess',onRoute);window.addEventListener('pagehide',stop);window.addEventListener('pageshow',e=>{if(e.persisted){revealedCid='';start();}});if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
})();

(() => {
  ...
  const generating=()=>document.documentElement.dataset.ng8Running==='1'||['loading','waiting','thinking','executing'].includes(document.documentElement.dataset.ng86Activity||'')||document.documentElement.dataset.ng105Verification==='1';
  ...
  function refreshCacheProjectIds(raw){
    const hidden=new Set((config.hiddenProjectIds||[]).map(normalizePid));
    cacheProjectIds=[...new Set((raw?.projects||[]).filter(p=>String(p?.id||'').startsWith('g-p-')&&!p?.domOnly&&p?.name).map(p=>normalizePid(p.id)).filter(id=>id&&!hidden.has(id)))];
  }
  async function loadState(){
    try{
      const raw=await chrome.storage.local.get([GOV_KEY,SETTINGS_KEY,CACHE_KEY]);
      if(raw[GOV_KEY])config={...config,...raw[GOV_KEY]};
      if(raw[SETTINGS_KEY])settings={...settings,...raw[SETTINGS_KEY]};
      refreshCacheProjectIds(raw[CACHE_KEY]||{});
    }catch{}
  }
  ...
  function desiredIds(){
    const hidden=new Set((config.hiddenProjectIds||[]).map(normalizePid));
    const explicit=(config.coreProjectIds||[]).map(normalizePid).filter(id=>id&&!hidden.has(id));
    // Self-heal window: if governance has temporarily lost its core list but the canonical
    // Project inventory is already cached, keep the native pin target coherent instead of 0/0.
    return new Set((explicit.length?explicit:cacheProjectIds).filter(id=>id&&!hidden.has(id)));
  }
  function patchDiagnostic(result=null){
    const diag=document.querySelector('#ng8-panel .ng8-diag');let valueText='';let klass='wait';
    if(!syncEnabled()){valueText=settings.safeMode||document.documentElement.dataset.ng90Safe==='1'?'PAUSE · SAFE MODE':'OFF · synchro native désactivée';}
    else{
      const desired=desiredIds();
      if(!desired.size){valueText='ATTENTE · inventaire/gouvernance Projects';}
      else if(result){valueText=`CORE · ${result.ok}/${result.desired} natifs${result.failed?` · ${result.failed} échec(s)`:''}`;klass=result.failed?'wait':'ok';}
      else{const current=nativePinnedIds(),ok=[...desired].filter(id=>current.has(id)).length;valueText=`CORE · ${ok}/${desired.size} natifs`;klass=ok===desired.size?'ok':'wait';}
    }
    window.__NIAKGPT_DIAGNOSTICS__?.set('pins',valueText);
    if(!diag)return;let row=diag.querySelector(':scope>.ng90-pins-diagnostic');if(!row){row=document.createElement('div');row.className='ng90-pins-diagnostic';diag.prepend(row);}const value=document.createElement('b'),label=document.createElement('span');label.textContent='pins';value.textContent=valueText;value.className=klass;row.replaceChildren(label,value);
  }
  ...
  async function reconcile(){
    ...
    try{
      await loadState();if(!canSync()){patchDiagnostic();return;}const desired=desiredIds();if(!desired.size){patchDiagnostic();return;}const before=nativePinnedIds();let failed=0;
  ...
  chrome.storage.onChanged.addListener((changes,area)=>{
    ...
    if(changes[GOV_KEY])config={...config,...(changes[GOV_KEY].newValue||{})};
    if(changes[SETTINGS_KEY])settings={...settings,...(changes[SETTINGS_KEY].newValue||{})};
    if(changes[CACHE_KEY])refreshCacheProjectIds(changes[CACHE_KEY].newValue||{});
    if(changes[GOV_KEY]||changes[SETTINGS_KEY]||changes[CACHE_KEY])schedule(800);
  });
  ...
  loadState().then(()=>{bindSidebar();patchDiagnostic();schedule(4200);});
})();

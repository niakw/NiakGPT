(() => {
  ...
  const heavy=()=>document.documentElement.dataset.ng8Heavy==='1';const can=()=>!conversationQuiet()&&!ratePaused()&&!document.hidden&&!document.documentElement.dataset.ng100Recovery&&document.documentElement.dataset.ng8Running!=='1'&&!['loading','waiting','thinking','executing'].includes(document.documentElement.dataset.ng86Activity||'')&&document.documentElement.dataset.ng90Safe!=='1';
  ...
  const unassigned=c=>!clean(c?.projectId)&&!projectIdFromHref(c?.href);
  const recentUnassigned=c=>{if(!unassigned(c))return false;const at=chatUpdatedMs(c);return !!at&&Date.now()-at>=0&&Date.now()-at<=RECENT_CATCHUP_MS;};
  function historicalCatchupReady(raw,targets){
    if(!raw||Number(raw.serverIndexedAt||0)<=0||!targets?.length)return false;
    const indexed=new Set((raw.indexedProjectIds||[]).filter(id=>String(id||'').startsWith('g-p-')));
    return targets.every(p=>indexed.has(p.id));
  }
  const needsClassification=(c,queueIds,catchup=false)=>queueIds.has(c?.projectId)||recentUnassigned(c)||(catchup&&unassigned(c));
  ...
  async function run(){
    ...
    try{
      ...
      if(!targets.length){
        ...
        window.__NIAKGPT_DIAGNOSTICS__?.set('reclassement',`ATTENTE · index Projects canonique requis · ${unassignedCount} chat(s) cache à rattraper`);
        return;
      }
      ...
      const stillQueued=new Set(allChats(raw).filter(c=>needsClassification(c,queueIds,catchup)).map(c=>c.id));for(const id of Object.keys(state.attempts))if(!stillQueued.has(id))delete state.attempts[id];for(const id of Object.keys(state.firstSeen))if(!stillQueued.has(id))delete state.firstSeen[id];
      ...
      const remaining=allChats(raw).filter(c=>needsClassification(c,queueIds,catchup)&&!locks[c.id]).length;
      ...
      const scope=catchup?' · historique complet':'';
      window.__NIAKGPT_DIAGNOSTICS__?.set('reclassement',moved?`OK · ${moved} reclassé${moved>1?'s':''} · ${remaining} restant${remaining>1?'s':''}${scope}`:waiting?`ATTENTE · ${remaining} à classer/rattraper · ${waiting} ambigu${waiting>1?'s':''}${evidence}${scope}`:`ATTENTE · ${remaining} à classer/rattraper${scope}`);

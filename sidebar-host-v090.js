(() => {
  ...
  const GENERIC=new Set(['design','ai','ia','coding','code','development','web development','technology','tech','social','social media','writing','general knowledge','general','e-commerce','ecommerce','seo','marketing','business','creative','research','productivity','other','misc','work','education','health','finance','home','cars','gaming','movies','food','personal development','studio','research lab','a classer','hors projet']);
  ...
  function hasTerm(text,key){const k=norm(key);if(!k)return false;return new RegExp(`(^|[^\\p{L}\\p{N}])${regexEsc(k)}(?=$|[^\\p{L}\\p{N}])`,'u').test(text);}
  ...
  async function indexVisible(root){
    ...
    try{await chrome.storage.local.set({[CACHE_KEY]:next});window.__NIAKGPT_DIAGNOSTICS__?.set('domindex',`OK · ${next.projects.length} projets · ${next.chats.length} chats`);}catch(e){window.__NIAKGPT_DIAGNOSTICS__?.set('domindex',`ERREUR · ${String(e?.message||e).slice(0,60)}`);}
  }
  ...
  async function fetchProjectsFresh(){
    ...
    for(let page=0;page<100;page++){
      ...
      const r=await rpc(`/backend-api/gizmos/snorlax/sidebar?${qs}`);if(!r.ok)throw new Error(`Projets ${r.status||0} · ${r.error||'erreur'}`);
  ...
  function classifyChat(chat,named){
    ...
    const hit=named.filter(n=>text.includes(n.key)||(current===n.key&&!GENERIC.has(n.key))).sort((a,b)=>b.key.length-a.key.length)[0];if(hit)return hit.name;
    let best=null,bestScore=0;
    for(const base of BASE.slice(0,-1)){let score=0;for(const key of base.keys)if(hasTerm(text,key))score++;if(score>bestScore){bestScore=score;best=base.name;}}
  ...
  async function prepareAutoPlan(){
    setRebuildStatus('INVENTAIRE · projets');const projects=await fetchProjectsFresh(),all=new Map();
    ...
    setRebuildStatus('INVENTAIRE · chats hors projets');for(const c of await fetchGeneralFresh()){const old=all.get(c.id);all.set(c.id,{...c,projectId:old?.projectId||c.projectId||'',snippet:c.snippet||old?.snippet||''});}
  ...
  function renderPlan(plan){
    ...
    modal.innerHTML=`<div class="card"><header><div><small>RECONSTRUCTION AUTO · 0.9.11</small><h2>Reconstruction automatique complète</h2><span>${plan.chats.length} chats · ${plan.oldProjects.length} projets actuels · ${plan.targets.length} projets cibles</span></div><button data-close>×</button></header><div class="warn"><b>Séquence sûre :</b> tous les chats sont d'abord sortis et vérifiés. Ensuite seulement les anciens projets vides sont supprimés. <b>Les fichiers et instructions propres aux anciens projets seront définitivement perdus.</b></div><div class="grid"><section><h3>PROJETS RÉCURRENTS DÉTECTÉS</h3>${named}</section><section><h3>STRUCTURE CIBLE</h3>${targets}</section></div><div id="ng911-progress">APERÇU · aucune modification effectuée</div><footer><button data-close>Annuler</button><span></span><input class="confirm" data-confirm placeholder="Tape RECONSTRUIRE"><button class="danger" data-execute disabled>RECONSTRUCTION AUTO</button></footer></div>`;
  ...
  function renderProgress(state){
    ensureAutoStyle();let modal=modalNode();if(!modal){modal=document.createElement('div');modal.id='ng911-auto';document.body.appendChild(modal);}
    modal.innerHTML=`<div class="card"><header><div><small>RECONSTRUCTION AUTO · EN COURS</small><h2>Reconstruction en cours</h2><span>L’état est sauvegardé et la reconstruction reprend automatiquement après les navigations.</span></div></header><div id="ng911-progress">${esc(state.message||state.phase)}</div><div class="warn">En cas d'erreur, NiakGPT s'arrête. Une suppression de projet n'est autorisée qu'après vérification qu'il ne contient plus aucun chat.</div><footer><span></span><button data-abort>Arrêter après l'étape courante</button></footer></div>`;
  ...
  async function startAutoRebuild(plan){if(rebuildBusy)return;const state={schema:2,active:true,phase:'detach',index:0,plan,created:{},message:'DÉTACHEMENT · préparation',startedAt:Date.now(),failures:[]};await saveState(state);renderProgress(state);resumeAuto();}
  async function verifyChatProject(id,expected){for(let i=0;i<3;i++){const r=await rpc(`/backend-api/conversation/${encodeURIComponent(id)}`,{timeout:14000});if(r.ok){const got=clean(r.data?.gizmo_id||r.data?.conversation_mode?.gizmo_id||'');if(got===expected)return true;}await sleep(220*(i+1));}return false;}
  async function moveChat(id,targetId){const expected=targetId||'',r=await rpc(`/backend-api/conversation/${encodeURIComponent(id)}`,{method:'PATCH',body:{gizmo_id:targetId||null},timeout:16000});if(!r.ok)return false;const ack=clean(r.data?.gizmo_id||r.data?.conversation_mode?.gizmo_id||'');if(ack===expected)return true;return verifyChatProject(id,expected);}
  ...
  async function deleteCurrentProject(project){
    if(!(await projectIsEmpty(project)))throw new Error(`BLOCAGE SÉCURITÉ · ${project.name} contient encore des chats`);const menu=await waitFor(()=>moreButton(project.name),8000);if(!menu)throw new Error(`Menu du projet introuvable · ${project.name}`);menu.click();
  ...
  async function resumeAuto(){
    ...
    try{
      if(state.phase==='detach'){
        while(state.index<state.plan.chats.length){const start=state.index,batch=state.plan.chats.slice(start,start+MOVE_CONCURRENCY);state.message=`SÉCURISATION · ${start+1}-${start+batch.length}/${state.plan.chats.length}`;await saveState(state);setRebuildStatus(state.message);const results=await Promise.all(batch.map(async c=>!c.projectId||await moveChat(c.id,'')));const failed=results.findIndex(ok=>!ok);if(failed>=0)throw new Error(`Impossible de sortir le chat ${batch[failed].title}`);state.index=start+batch.length;await saveState(state);}
        for(const p of state.plan.oldProjects)if(!(await projectIsEmpty(p)))throw new Error(`BLOCAGE SÉCURITÉ · ${p.name} n’est pas vide`);state.phase='delete';state.index=0;state.message='SUPPRESSION · projets vides';await saveState(state);
      }
      ...
      if(state.phase==='create'){
        if(!/^\/projects\/?$/.test(location.pathname)){location.assign('/projects');rebuildBusy=false;return;}while(state.index<state.plan.targets.length){const t=state.plan.targets[state.index];state.message=`CRÉATION · ${state.index+1}/${state.plan.targets.length} · ${t.name}`;await saveState(state);setRebuildStatus(state.message);const made=await createProjectDOM(t.name);state.created[t.name]=made.id;state.index++;await saveState(state);await sleep(250);}state.phase='assign';state.index=0;state.message='CLASSEMENT · conversations';await saveState(state);
      }
      if(state.phase==='assign'){
        while(state.index<state.plan.assignments.length){const start=state.index,batch=state.plan.assignments.slice(start,start+MOVE_CONCURRENCY);for(const a of batch)if(!state.created[a.targetName])throw new Error(`Projet cible absent · ${a.targetName}`);state.message=`CLASSEMENT · ${start+1}-${start+batch.length}/${state.plan.assignments.length}`;await saveState(state);setRebuildStatus(state.message);const results=await Promise.all(batch.map(a=>moveChat(a.id,state.created[a.targetName])));const failed=results.findIndex(ok=>!ok);if(failed>=0)throw new Error(`Déplacement non vérifié · ${batch[failed].title}`);state.index=start+batch.length;await saveState(state);}
        state.phase='done';state.active=false;state.message=`TERMINÉ · ${state.plan.assignments.length} chats · ${state.plan.targets.length} projets`;state.finishedAt=Date.now();await saveState(state);setRebuildStatus(state.message);try{await chrome.storage.local.remove(CACHE_KEY);}catch{}renderProgress(state);
      }
    }catch(e){await failState(state,e);return;}finally{if((await readState())?.active!==true)rebuildBusy=false;}
  }

  function patchAutoButton(){const panel=document.getElementById('ng8-panel');if(!panel)return;const actions=panel.querySelector('.ng8-actions');if(actions&&!actions.querySelector('[data-ng911-auto]')){const b=document.createElement('button');b.type='button';b.dataset.ng911Auto='1';b.className='ng911-auto-button';b.textContent='RECONSTRUCTION AUTO';b.title='Analyser tous les chats puis reconstruire automatiquement les projets';b.addEventListener('click',openAuto);actions.appendChild(b);}}

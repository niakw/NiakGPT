(() => {
  ...
  function rootMutationRelevant(records){
    for(const record of records)for(const node of [...record.addedNodes,...record.removedNodes]){
      if(!(node instanceof Element))continue;
      if(node===navNode||node.contains?.(navNode)||node.matches?.(SIDEBAR_SEL)||node.querySelector?.(SIDEBAR_SEL))return true;
    }
    return false;
  }
  function kindFor(el){
  ...
  function start(){bind();decorate();rootObserver?.disconnect();rootObserver=new MutationObserver(records=>{if(!rootMutationRelevant(records))return;const next=navRoot();if(next!==navNode){bind(next);decorate();}});rootObserver.observe(document.documentElement,{childList:true,subtree:true});}

(() => {
  'use strict';
  if(location.hostname!=='chatgpt.com'||window.__NIAKGPT_PROJECTS_AUTHORITY_111__)return;
  window.__NIAKGPT_PROJECTS_AUTHORITY_111__=true;
  ...
  const HIDE='ng111-native-projects-authoritative';
  ...
  function labelNodes(scope){
    if(!scope)return[];
    const found=[];
    for(const el of scope.querySelectorAll('h1,h2,h3,[role="heading"],button,[role="button"],a[href],[aria-label]')){
  ...
  function hasProjectSurface(scope){
    if(!scope)return false;
    if(scope.querySelector('[class*="project-unfurl-row"],a[href*="/g/g-p-"]'))return true;
    return [...scope.querySelectorAll('a[href]')].some(a=>projectHomeHref(a.getAttribute('href')));
  }
  ...
  function isDedicatedExpando(section,nav){
    ...
    return labelNodes(section).length>0&&hasProjectSurface(section);
  }

  function compactLabelTarget(label,nav){
    if(!label||label===nav)return null;
    if(label.matches?.('a[href]'))return label;
  ...
  function projectOnlySibling(target,nav){
    ...
    if(!hasProjectSurface(sibling))return null;
    if(sibling.querySelector('h1,h2,h3,[role="heading"]'))return null;
    const nonProjectLinks=[...sibling.querySelectorAll('a[href]')].some(a=>!projectHref(a.getAttribute('href')));
    return nonProjectLinks?null:sibling;
  }

  function projectHomeTarget(link,nav){
    if(!link||link===nav||!outsideOwn(link))return null;
    const row=link.closest('[data-sidebar-item="true"],li');
    if(row&&row!==nav&&outsideOwn(row)&&!row.contains(ownProjects())){
      const interactive=row.querySelectorAll('a[href],button,[role="button"]');
      if(interactive.length===1)return row;
    }
    return link;
  }
  ...
  function nativeTargets(nav){
    ...
    // ChatGPT can expose Projects as a standalone /projects navigation row above the
    // managed NiakGPT block. It is not necessarily a heading or a g-p Project row.
    for(const link of nav.querySelectorAll('a[href]')){
      if(!outsideOwn(link)||insideDedicated(link)||!projectHomeHref(link.getAttribute('href')))continue;
      const target=projectHomeTarget(link,nav);
  ...
  function release(){
    for(const el of document.querySelectorAll('.'+HIDE)){
      el.classList.remove(HIDE);el.removeAttribute('data-ng111-native-projects');el.removeAttribute('aria-hidden');
    }
  ...
  function apply(){
    ...
    for(const target of targets){target.classList.add(HIDE);target.dataset.ng111NativeProjects='1';target.setAttribute('aria-hidden','true');}
    for(const old of nav.querySelectorAll('.'+HIDE))if(!targets.includes(old)){old.classList.remove(HIDE);old.removeAttribute('data-ng111-native-projects');old.removeAttribute('aria-hidden');}

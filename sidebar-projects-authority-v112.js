(() => {
  ...
  function compactLabelTarget(label,nav){if(!label||label===nav)return null;if(label.matches?.('a[href]'))return label;const button=label.closest('button,[role="button"]');if(button&&button!==nav&&outsideOwn(button))return button;return label;}
  ...
  function projectHomeTarget(link,nav){if(!link||link===nav||!outsideOwn(link))return null;const row=link.closest('[data-sidebar-item="true"],li');if(row&&row!==nav&&outsideOwn(row)&&!row.contains(ownProjects())){const interactive=row.querySelectorAll('a[href],button,[role="button"]');if(interactive.length===1)return row;}return link;}

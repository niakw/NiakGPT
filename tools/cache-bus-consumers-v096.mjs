...

{
  ...
  if(!s.includes("const cacheBusText=read('cache-bus-v096.js')")){
    ...
    s=s.replace("for(const file of ['chronology-v090.js','pin-folders-v096.js','hotcache-v084.js','project-governance-v090.js','multitab-v090.js'])no(texts[file],'chrome.storage.local.get(CACHE_KEY)'","for(const file of ['app-v090.js','chronology-v090.js','pin-folders-v096.js','hotcache-v084.js','project-governance-v090.js','multitab-v090.js'])no(texts[file],'chrome.storage.local.get(CACHE_KEY)'");
  }
  write(p,s);
}

{
  ...
  s=s.replace("(bus?await bus.get():(await chrome.storage.local.get(CACHE_KEY))[CACHE_KEY])","(bus?await bus.get():null)");
  ...
  s=s.replace("if(changes[CACHE_KEY]&&!S.indexing){loadCache().then(()=>{decorateSidebar();renderPanel();});}","if(changes[CACHE_KEY]&&!S.indexing){loadCache(changes[CACHE_KEY].newValue).then(()=>{decorateSidebar();if(S.panelOpen)renderPanel();});}");
  must(s.includes('window.__NIAKGPT_CACHE_BUS__'),'app cache bus missing');must(!s.includes('chrome.storage.local.get(CACHE_KEY)'),'app direct cache read remains');must(s.includes('loadCache(changes[CACHE_KEY].newValue)'),'app cache storage race remains');write(p,s);
}

{
  ...
  s=s.replace('  readCache();bindSidebar();','  if(!cacheBus)readCache({});bindSidebar();');must(s.includes('__NIAKGPT_CACHE_BUS__'),'chronology cache bus missing');must(!s.includes('chrome.storage.local.get(CACHE_KEY)'),'chronology direct cache read remains');write(p,s);
}

{
  const p='pin-folders-v096.js';let s=read(p);
  if(!s.includes('function projectSnapshotSignature(')){
    const marker='  function setOpen(pid){';must(s.includes(marker),'pin folder signature marker missing');s=s.replace(marker,`  function projectSnapshotSignature(raw,pid){if(!pid)return'';const chats=(raw?.chats||[]).filter(c=>c?.projectId===pid).map(c=>[c.id,c.updated||c.update_time||0,c.title||'']);return JSON.stringify([raw?.counts?.[pid]??null,chats]);}\n  function acceptCache(next){const before=projectSnapshotSignature(cache,openPid);cache=next&&typeof next==='object'?next:cache;const after=projectSnapshotSignature(cache,openPid);if(!observedBox)bindBox();if(openPid&&before!==after)schedule(40);else if(!openPid)schedule(80);}\n${marker}`);
  }
  ...
  if(s.includes(tail))s=s.replace(tail,replacement);s=s.replace('  loadCache();bootstrap();','  bootstrap();');must(s.includes('__NIAKGPT_CACHE_BUS__'),'pin folder cache bus missing');must(!s.includes('chrome.storage.local.get(CACHE_KEY)'),'pin folder direct cache read remains');write(p,s);
}

{
  const p='hotcache-v084.js';let s=read(p);
  if(!s.includes('let pendingCache = null;'))s=s.replace('  let syncTimer = 0;','  let syncTimer = 0;\n  let pendingCache = null;');
  s=s.replace('  async function syncMeta() {\n    clearTimeout(syncTimer);\n    syncTimer = 0;\n    try {\n      const raw = (await chrome.storage.local.get(CACHE_KEY))[CACHE_KEY] || {};',`  async function syncMeta(rawOverride) {clearTimeout(syncTimer);syncTimer=0;try {const raw=rawOverride&&typeof rawOverride==='object'?rawOverride:{};`);
  ...
  s=s.replace('  scheduleMeta(50);','  if(!cacheBus)scheduleMeta({},50);');must(s.includes('__NIAKGPT_CACHE_BUS__'),'hotcache metadata cache bus missing');must(!s.includes('chrome.storage.local.get(CACHE_KEY)'),'hotcache isolated direct cache read remains');write(p,s);
}

{
  ...
  if(s.includes(old))s=s.replace(old,next);s=s.replace("if(from&&Number.isFinite(Number(cache.counts[from])))cache.counts[from]=Math.max(0,Number(cache.counts[from])-1);","if(from&&cache.counts[from]!=null&&Number.isFinite(Number(cache.counts[from])))cache.counts[from]=Math.max(0,Number(cache.counts[from])-1);");s=s.replace("if(target&&target!==from&&Number.isFinite(Number(cache.counts[target])))cache.counts[target]=Number(cache.counts[target])+1;","if(target&&target!==from&&cache.counts[target]!=null&&Number.isFinite(Number(cache.counts[target])))cache.counts[target]=Number(cache.counts[target])+1;");must(s.includes('__NIAKGPT_CACHE_BUS__'),'Governance cache bus missing');must(!s.includes('chrome.storage.local.get(CACHE_KEY)'),'Governance direct cache read remains');must(s.includes('cache.counts[from]!=null'),'Governance unknown source count can be corrupted');write(p,s);
}

{
  const p='multitab-v090.js';let s=read(p);s=s.replace("    let raw={};try{raw=(await chrome.storage.local.get(CACHE_KEY))[CACHE_KEY]||{};}catch{}","    let raw={};try{raw=(await window.__NIAKGPT_CACHE_BUS__?.get())||{};}catch{}");must(s.includes('__NIAKGPT_CACHE_BUS__'),'CLIENT Quick Open cache bus missing');must(!s.includes('chrome.storage.local.get(CACHE_KEY)'),'CLIENT Quick Open direct cache read remains');write(p,s);
}

...
must(s.includes('incoming?.at===S.lastCacheWriteAt'),'core cache write feedback guard missing');
fs.writeFileSync(p,s);

const cp='tools/check-hotpath-v096.mjs';let c=fs.readFileSync(cp,'utf8');
if(!c.includes('cache feedback guard'))c=c.replace('// Project indexing batches full storage writes and never flushes them during activity.',"// Core cache writes must notify consumers without causing the core to rebuild its own state.\nhas(app,'lastCacheWriteAt:0','cache feedback guard state missing');\nhas(app,'incoming?.at===S.lastCacheWriteAt','cache feedback guard missing');\n\n// Project indexing batches full storage writes and never flushes them during activity.");

...
if(manifest.version!=='0.9.107')fail(`unexpected release ${manifest.version}`);
...
for(const token of ['async function probeReactHydration','chrome.scripting.executeScript','world:\'MAIN\'','niakgpt:probe-react-hydration-v106','containerFound','rootSettled','ownedCount'])need(background,token,'MAIN-world React hydration probe contract incomplete');
...
for(const token of ['waitDomInteractive','waitForChatShell','restorePendingContinuity','guardUpdateOnboarding','injectRuntime','for(const delay of [0,240,720])','safeToMutate=!!document.body','waitForQuiet(1200,7000)','waitStableHostIdentity(1600,8500)','idleTurn(2200)','requestIdleCallback','mainWorldReactProbe','niakgpt:probe-react-hydration-v106','waitReactHydrationOwnership','waitTrustedHydratedInteraction','react-main-world-settled','trusted-interaction-after-host-fault','hydrationFault','containerFound','ng100HydrationProof','__NIAKGPT_HOST_HYDRATED_100__','niakgpt:host-hydrated-v100'])need(gate,token,'late-scheduler/full-document React hydration bootstrap contract incomplete');

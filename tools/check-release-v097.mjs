...
// Parser-safe boot: heavy DOM activity starts only after the document is parsed.
...
// Shared cache bus: one large storage read per tab, then subscriptions.
...
// Diagnostic bus owns hot-cache metadata/status after parsing.
...
// Core stays event-driven and yields on long conversations.
...
// Activity states are network-led and observers exist only while activity is live.
...
// One multi-tab WORKER; background coordination never scans conversation turns.
...
// Manual Project decisions outrank automation.
...
// Projects and native pins remain verified/folder-like.
...
// Native side panels coexist with the NiakGPT rail without generation-time scans.
...
// Hot conversation cache is bounded in disk/RAM, fresh and cross-tab deduplicated.
...
// Runtime must remain local-only except ChatGPT itself.
...
console.log(`NiakGPT ${manifest.version} unified release invariants: OK`);

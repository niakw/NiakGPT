...
for(const script of manifest.content_scripts||[]){for(const file of script.js||[])runtime.add(file);for(const file of script.css||[])runtime.add(file);}
...
for(const file of runtime){
  const src=path.join(root,file);if(!fs.existsSync(src))throw new Error(`Missing packaged file: ${file}`);
  const dest=path.join(build,file);fs.mkdirSync(path.dirname(dest),{recursive:true});fs.copyFileSync(src,dest);
}

const dead=['app-v08-safe.js','multitab-v083.js','polish-v081.js','chronology-v081.js','project-governance-v085.js','project-pins-v085.js'];
...
for(const file of [...runtime].filter(x=>/\.(js|css|json|md)$/i.test(x))){const text=fs.readFileSync(path.join(build,file),'utf8');if(sensitive.test(text))throw new Error(`Personal data leaked into package: ${file}`);}

...
assert.match(runtime, /retryPileRows/);
assert.match(runtime, /retryFailedChatsNow/);
assert.match(runtime, /manual:true/);
assert.match(runtime, /holdReason:'rate-limit-manual'/);
assert.match(runtime, /releaseQueueHold/);
assert.doesNotMatch(runtime, /chat-fetch-retry|CHAT_RETRY_BACKOFF_MS|queueWait/,'failed chats must not remain on an automatic retry timer');
...
assert.match(runtime, /const inventoryGaps=list=>/);
assert.match(runtime, /function inventoryGapSignature\(list\)/);
assert.match(runtime, /inventoryGapAttempts/);
assert.match(runtime, /inventoryStalledCacheSignature/);
assert.match(runtime, /pauseReason:'inventory-stalled'/);
assert.match(runtime, /mode:pending\.length\?'queued':'idle'/);
...
assert.match(ui, /écart d’inventaire stable · aucune relance automatique/);
assert.match(ui, /reprise uniquement si le cache change/);
...
assert.match(ui, /Réessayer les chats en échec/);
assert.match(ui, /data-ng132-retry-failed/);
assert.match(ui, /Reprendre après restriction ChatGPT/);
assert.match(ui, /Pile manuelle/);
...
assert.ok(fs.existsSync('visual-lab/project-memory-manual-rate-hold-v121.mjs'),'Project Memory real-429 manual-resume gate missing');

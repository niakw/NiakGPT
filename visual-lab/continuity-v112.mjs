...
for(const [engine,launcher] of Object.entries(engines)){
  ...
  try{
    ...
    assert(openedProject===projectId,'continuation did not pre-open the exact Project drawer');
    ...
    const result=await page.evaluate(({newId,projectId})=>{
      ...
      return{patches:window.__patches,chat:cache.chats.find(c=>c.id===newId),projectChat:(cache.projectChats?.[projectId]||[]).find(c=>c.id===newId),count:cache.counts?.[projectId],indexed:(cache.indexedProjectIds||[]).includes(projectId),openKey:sessionStorage.getItem('niakgpt-open-pin-folder-v096'),lock:gov.locks?.[newId],pending:sessionStorage.getItem('niakgpt-continuity-pending-v100'),projectId};
    },{newId,projectId});
    ...
    assert(result.chat?.projectId===projectId,'new continued chat was not inserted into cache with exact Project');
    assert(result.projectChat?.projectId===projectId,'new continued chat missing from Project drawer inventory');
    ...
    assert(result.openKey===projectId,'continued chat did not keep Project drawer open');

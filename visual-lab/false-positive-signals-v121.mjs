...
const [continuityJs,interruptJs]=await Promise.all([
...
const P='g-p-niakgpt';
const html=`<!doctype html><html lang="fr"><head><meta charset="utf-8"><style>body{font-family:Arial}aside,main{display:block}#ng119-interruption{position:fixed;bottom:20px;left:20px;padding:10px;background:#111;color:#fff}.hidden{display:none}</style></head><body>
<aside data-testid="conversation-sidebar"><a id="chat-link" href="/g/${P}/c/${C}">Tests de régression</a></aside>
...
for(const [engine,launcher] of Object.entries(engines)){
  ...
  try{
    await page.addInitScript(({C,P})=>{
      const listeners=[];
      const store={
        'niakgpt-v08-cache':{projects:[{id:P,name:'NiakGPT'}],chats:[{id:C,title:'Tests de régression',projectId:P}],projectChats:{[P]:[]}},
    ...
    console.log(`${engine} false-positive signals: PASS`);
  } finally {
    await browser.close();
  }
}

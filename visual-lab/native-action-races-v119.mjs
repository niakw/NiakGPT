...
for(const [engine,launcher] of Object.entries(engines)){
  ...
  try{
    // pagehide/BFCache during the hover delay must invalidate the old async action.
    {
    ...
    // A React remount can clone aria-busy from the in-flight button. The stale action
    // must abort before clicking native UI and the new button must be normalized clean.
    {
    ...
    // With aria-controls available, only the controlled menu belongs to the action.
    // A different role=menu appearing after the trigger click must remain untouched.
    {
    ...
    // If ChatGPT markup drifts and the row has only a clearly unrelated textual
    // button, NiakGPT must fail closed instead of clicking buttons.at(-1).
    {

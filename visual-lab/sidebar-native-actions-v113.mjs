...
for(const [engine,launcher] of Object.entries(engines)){
  ...
  try{
    ...
    await page.locator('.ng113-native-actions-project').first().click();await page.waitForFunction(()=>document.querySelector('[role=menu]')?.dataset.kind==='project',null,{timeout:1800});let projectMenu=await page.evaluate(()=>({kind:document.querySelector('[role=menu]')?.dataset.kind||'',items:[...document.querySelectorAll('[role=menuitem]')].map(x=>x.textContent)}));
    ...
    await page.locator('.ng113-native-actions-chat').click();await page.waitForFunction(()=>document.querySelector('[role=menu]')?.dataset.kind==='chat',null,{timeout:1800});let chatMenu=await page.evaluate(()=>({kind:document.querySelector('[role=menu]')?.dataset.kind||'',items:[...document.querySelectorAll('[role=menuitem]')].map(x=>x.textContent)}));

...
for(const [engine,launcher] of Object.entries(engines)){
  ...
  try{
    ...
    await page.evaluate(()=>{document.getElementById('limit-alert')?.remove();document.getElementById('ng119-interruption')?.remove();window.__ng119RetryClicks=0;const b=document.createElement('button');b.id='native-retry';b.textContent='Réessayer';b.onclick=()=>window.__ng119RetryClicks++;document.querySelector('main').appendChild(b);const a=document.createElement('div');a.id='network-alert';a.setAttribute('role','alert');a.textContent='Connexion interrompue. En attente de la réponse complète.';document.getElementById('signal-host').appendChild(a);});
    await page.waitForFunction(()=>document.querySelector('#ng119-interruption[data-type="network"]'),null,{timeout:2500});
    const recoveryOwnSurface=await page.evaluate(()=>({text:document.querySelector('#ng119-interruption')?.textContent||'',type:document.querySelector('#ng119-interruption')?.dataset.type||''}));assert(recoveryOwnSurface.type==='network'&&/CONNEXION INTERROMPUE/i.test(recoveryOwnSurface.text),'network recovery surface missing its status text');
    await page.evaluate(()=>{document.getElementById('network-alert')?.remove();window.dispatchEvent(new Event('online'));});
    await page.waitForFunction(()=>!document.getElementById('ng119-interruption')||document.querySelector('#ng119-interruption[data-type="network"] .ng119-ready'),null,{timeout:3500}).catch(()=>{});
    await page.waitForTimeout(450);
    assert(await page.evaluate(()=>window.__ng119RetryClicks)===0,'network recovery must never auto-click ChatGPT native Retry');

    await page.evaluate(()=>{document.getElementById('ng119-interruption')?.remove();window.__ng119ChallengeClicks=0;const a=document.createElement('div');a.id='verify-alert';a.setAttribute('aria-live','polite');a.textContent='Nos systèmes effectuent quelques vérifications ….';a.onclick=()=>window.__ng119ChallengeClicks++;document.getElementById('signal-host').appendChild(a);});
    await page.waitForFunction(()=>document.querySelector('#ng119-interruption[data-type="verify"]'),null,{timeout:2500});
    await page.waitForTimeout(350);
    assert(await page.evaluate(()=>window.__ng119ChallengeClicks)===0,'verification challenge was interacted with instead of being respected');
    assert(await page.evaluate(()=>document.documentElement.dataset.ng105Verification)==='1','exact ChatGPT verification text did not pause NiakGPT');
    await page.evaluate(()=>document.getElementById('verify-alert')?.remove());
    await page.waitForFunction(()=>document.documentElement.dataset.ng105Verification!=='1',null,{timeout:4500});

    // No native retry: preserve the last partial assistant output and prepare a continuity prompt without auto-send.
    await page.evaluate(()=>{
      document.getElementById('native-retry')?.remove();
      document.getElementById('ng119-interruption')?.remove();
      const ed=document.getElementById('prompt-textarea');ed.value='';ed.dispatchEvent(new InputEvent('input',{bubbles:true,data:null}));
      if(!document.getElementById('assistant-partial')){const partial=document.createElement('div');partial.id='assistant-partial';partial.dataset.messageAuthorRole='assistant';partial.textContent='Voici une réponse partielle utile qui doit survivre à une coupure réseau. Dernier point validé : conserver exactement la continuité du travail.';document.querySelector('main').insertBefore(partial,document.getElementById('prompt-textarea'));}
      const a=document.createElement('div');a.id='network-alert-2';a.setAttribute('role','alert');a.textContent='Connexion interrompue. En attente de la réponse complète.';document.getElementById('signal-host').appendChild(a);
    });
    await page.waitForFunction(()=>document.querySelector('#ng119-interruption[data-type="network"]'),null,{timeout:2500});
    await page.evaluate(()=>{document.getElementById('network-alert-2')?.remove();window.dispatchEvent(new Event('online'));});
    await page.waitForFunction(()=>document.querySelector('#ng119-interruption[data-type="network"] [data-ng119-resume]'),null,{timeout:5000});
    await page.locator('#ng119-interruption [data-ng119-resume]').click();
    const continuity=await page.locator('#prompt-textarea').inputValue();
    assert(/réponse précédente a été interrompue/i.test(continuity),'partial-response continuity prompt not prepared');
    assert(/Dernier point validé : conserver exactement la continuité du travail/i.test(continuity),'last partial assistant tail was not preserved');
    assert(await page.locator('[data-message-author-role="user"]').count()===0,'continuity recovery auto-sent a user message');

    console.log(`${engine} stable pins/native-only actions/interruption recovery + exact verification/network messages: PASS`);

...
for(const [engine,launcher] of Object.entries(engines)){
  ...
  try{
    ...
    for(const route of ['/','/library','/projects']){
      await page.evaluate(route=>{history.pushState({},'',route);dispatchEvent(new PopStateEvent('popstate'));},route);await page.waitForTimeout(130);
    ...
    await page.locator('#prompt-textarea').fill('brouillon à préserver');
    ...
    await page.evaluate(()=>{document.getElementById('network-alert')?.remove();window.dispatchEvent(new Event('online'));});await page.waitForFunction(()=>window.__ng119RetryClicks===1,null,{timeout:4500});await page.waitForTimeout(450);assert(await page.evaluate(()=>window.__ng119RetryClicks)===1,'native network Retry was triggered more than once or recovery UI self-detected as a network error');assert(await page.locator('#prompt-textarea').inputValue()==='brouillon à préserver','network interruption lost the composer draft');
    ...
    await page.waitForFunction(()=>document.querySelector('#ng119-interruption[data-type="verify"]')&&document.documentElement.dataset.ng105Verification==='1',null,{timeout:2500});await page.waitForTimeout(300);assert(await page.evaluate(()=>window.__ng119ChallengeClicks)===0,'verification challenge was interacted with instead of being respected');
    await page.evaluate(()=>document.getElementById('verify-alert')?.remove());await page.waitForFunction(()=>document.documentElement.dataset.ng105Verification!=='1',null,{timeout:3500});await page.waitForFunction(()=>window.__ng119RetryClicks===2,null,{timeout:4500});await page.waitForTimeout(350);assert(await page.evaluate(()=>window.__ng119RetryClicks)===2,'post-verification native Retry did not run exactly once');

    console.log(`${engine} 0.9.70 stable pins/native-only actions/verification+network recovery: PASS`);
  }finally{await context.close();await browser.close();}
}
console.log(`sidebar-recovery-v119: ${Object.keys(engines).join(',')} PASS`);
await import('./chat-drawer-active-v120.mjs');
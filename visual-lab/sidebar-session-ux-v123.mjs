...
for(const [engine,launcher] of Object.entries(engines)){
  ...
  try{
    ...
    await page.waitForFunction(()=>document.getElementById('ng8-pins')?.dataset.ng131Mounted==='1',null,{timeout:3000});
    await page.waitForFunction(()=>{const native=document.querySelector('.native');return !!native&&getComputedStyle(native).display==='none';},null,{timeout:3000});
    const placement=await page.evaluate(()=>{const box=document.getElementById('ng8-pins'),nav=box?.parentElement,native=nav?.querySelector('.native'),primary=nav?.querySelector(':scope>a[href="/"]'),recent=[...nav?.querySelectorAll?.('h3')||[]].find(x=>/Récents/i.test(x.textContent||'')),children=nav?[...nav.children]:[],br=box?.getBoundingClientRect(),sr=box?.closest('[data-testid="conversation-sidebar"]')?.getBoundingClientRect();return{mode:box?.dataset.ng121Placement||'',mountPolicy:box?.dataset.ng121MountPolicy||'',mountCount:box?.dataset.ng121MountCount||'',boxIndex:children.indexOf(box),nativeIndex:children.indexOf(native),primaryIndex:children.indexOf(primary),first:children[0]===box,nativeDisplay:native?getComputedStyle(native).display:'',recentDisplay:recent?getComputedStyle(recent).display:'',widthRatio:br&&sr?br.width/sr.width:0};});
    assert(placement.mountPolicy==='direct-once'&&placement.mountCount==='1',`${engine}: Projects block was not mounted directly once (${placement.mountPolicy}/${placement.mountCount})`);assert(placement.mode==='native-projects',`${engine}: Projects block did not lock to the native Projects slot (${placement.mode})`);assert(placement.primaryIndex>=0&&placement.primaryIndex<placement.boxIndex&&placement.boxIndex<placement.nativeIndex&&!placement.first,`${engine}: Projects block drifted above native primary/logo area`);assert(placement.nativeDisplay==='none',`${engine}: native ChatGPT Projects is still visible in the human UX session`);assert(placement.recentDisplay!=='none',`${engine}: generic Recents/Chats was hidden with native Projects`);assert(placement.widthRatio>.88,`${engine}: managed Projects does not span the sidebar lane (${placement.widthRatio})`);
    ...
    for(const sample of [0,Math.floor(projects.length/2),projects.length-1]){
      const row=page.locator('#ng8-pins .ng96-pin-entry').nth(sample);await row.evaluate(entry=>{const list=entry.closest('.ng8-pin-list');if(!list)return;const er=entry.getBoundingClientRect(),vr=list.getBoundingClientRect(),delta=((er.top+er.bottom)/2)-((vr.top+vr.bottom)/2);list.dispatchEvent(new WheelEvent('wheel',{bubbles:true,cancelable:true,deltaY:Math.sign(delta||1)*120}));list.scrollTop+=delta;});await page.waitForTimeout(120);
    ...
    await page.waitForFunction(()=>document.getElementById('ng8-pins')?.dataset.ng131Mounted==='1',null,{timeout:3000});
    await page.waitForFunction(()=>{const native=document.querySelector('[data-testid="conversation-sidebar"] .native');return !!native&&getComputedStyle(native).display==='none';},null,{timeout:3000});
    const recovered=await page.evaluate(()=>{const box=document.getElementById('ng8-pins'),nav=box?.parentElement,native=nav?.querySelector('.native'),primary=nav?.querySelector(':scope>a[href="/"]'),recent=[...nav?.querySelectorAll?.('h3')||[]].find(x=>/Récents/i.test(x.textContent||'')),children=nav?[...nav.children]:[],br=box?.getBoundingClientRect(),sr=box?.closest('[data-testid="conversation-sidebar"]')?.getBoundingClientRect();return{actions:document.querySelectorAll('#ng8-pins .ng113-native-actions-project').length,head:document.querySelector('#ng8-pins .ng8-pin-head')?.textContent||'',menus:document.querySelectorAll('#ng123-action-menu').length,placement:box?.dataset.ng121Placement||'',boxIndex:children.indexOf(box),nativeIndex:children.indexOf(native),primaryIndex:children.indexOf(primary),nativeDisplay:native?getComputedStyle(native).display:'',recentDisplay:recent?getComputedStyle(recent).display:'',widthRatio:br&&sr?br.width/sr.width:0};});assert(recovered.actions===projects.length&&/PROJECTS/.test(recovered.head)&&recovered.menus===0,`${engine}: sidebar remount did not recover a complete interactive catalog`);assert(recovered.placement==='native-projects'&&recovered.primaryIndex<recovered.boxIndex&&recovered.boxIndex<recovered.nativeIndex,`${engine}: sidebar remount recovered Projects at the wrong position`);assert(recovered.nativeDisplay==='none'&&recovered.recentDisplay!=='none'&&recovered.widthRatio>.88,`${engine}: remount reintroduced duplicate/half-width Projects: ${JSON.stringify(recovered)}`);

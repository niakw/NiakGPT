...
for(const [engine,launcher] of Object.entries(engines)){
  ...
  try{
    ...
    for(const sample of [0,Math.floor(projects.length/2),projects.length-1]){
      const row=page.locator('#ng8-pins .ng96-pin-entry').nth(sample);await row.evaluate(entry=>{const list=entry.closest('.ng8-pin-list');if(!list)return;const er=entry.getBoundingClientRect(),vr=list.getBoundingClientRect();list.scrollTop+=((er.top+er.bottom)/2)-((vr.top+vr.bottom)/2);});await page.waitForTimeout(80);

...
for(const [engine,launcher] of Object.entries(engines)){
  ...
  try{
    ...
    const projectRows=await page.evaluate(()=>{
      return [...document.querySelectorAll('#ng8-pins .ng96-pin-entry')].map(entry=>{
        ...
        return {
          ...
          visible:(()=>{const list=entry.closest('.ng8-pin-list'),vr=list?.getBoundingClientRect();return !!(lr&&vr&&lr.bottom>vr.top&&lr.top<vr.bottom);})(),
    ...
    for(const row of projectRows){
      ...
      if(row.visible)assert(row.linkHit&&row.buttonHit,`${engine}: visible Project row geometry is not actually hit-testable ${row.id}: ${JSON.stringify(row)}`);
    }
    for(const sample of [0,Math.floor(projects.length/2),projects.length-1]){
      const row=page.locator('#ng8-pins .ng96-pin-entry').nth(sample);await row.scrollIntoViewIfNeeded();await page.waitForTimeout(40);
      const hitState=await row.evaluate(entry=>{const link=entry.querySelector(':scope>a[data-ng8-pin]'),button=entry.querySelector(':scope>.ng113-native-actions-project'),lr=link.getBoundingClientRect(),br=button.getBoundingClientRect(),hit=(r,sel)=>!!document.elementFromPoint((r.left+r.right)/2,(r.top+r.bottom)/2)?.closest(sel);return{link:hit(lr,'a[data-ng8-pin]'),button:hit(br,'.ng113-native-actions-project')};});

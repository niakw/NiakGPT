...
for(const [engine,launcher] of Object.entries(engines)){
  ...
  try{
    await page.addInitScript(({cache,P1,P2})=>{
      ...
      window.chrome={runtime:{id:'lab',getManifest:()=>({version:'0.9.81'})},storage:{local:{get:async keys=>{if(typeof keys==='string')return{[keys]:keys==='niakgpt-v08-cache'?structuredClone(data):{}};return{'niakgpt-v08-cache':structuredClone(data),'niakgpt-governance-v085':{coreProjectIds:[],hiddenProjectIds:[]}};},set:async obj=>{if(obj['niakgpt-v08-cache'])publish(obj['niakgpt-v08-cache']);}},onChanged:{addListener:fn=>storageListeners.push(fn)}}};

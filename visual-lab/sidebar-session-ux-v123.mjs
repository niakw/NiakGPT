...
for(const [engine,launcher] of Object.entries(engines)){
  ...
  try{
    await page.addInitScript(({cache,P1,P2})=>{
      ...
      window.chrome={runtime:{id:'lab',getManifest:()=>({version:'0.9.82'})},storage:{local:{get:async keys=>{if(typeof keys==='string')return{[keys]:keys==='niakgpt-v08-cache'?structuredClone(data):{}};return{'niakgpt-v08-cache':structuredClone(data),'niakgpt-governance-v085':{coreProjectIds:[],hiddenProjectIds:[]}};},set:async obj=>{if(obj['niakgpt-v08-cache'])publish(obj['niakgpt-v08-cache']);}},onChanged:{addListener:fn=>storageListeners.push(fn)}}};
    ...
    const placement=await page.evaluate(()=>{const box=document.getElementById('ng8-pins'),nav=box?.parentElement,native=nav?.querySelector('.native'),primary=nav?.querySelector(':scope>a[href="/"]'),children=nav?[...nav.children]:[];return{mode:box?.dataset.ng121Placement||'',mountPolicy:box?.dataset.ng121MountPolicy||'',mountCount:box?.dataset.ng121MountCount||'',boxIndex:children.indexOf(box),nativeIndex:children.indexOf(native),primaryIndex:children.indexOf(primary),first:children[0]===box};});

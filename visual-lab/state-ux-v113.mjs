...
for(const [engine,launcher] of Object.entries(engines)){
  ...
  try{
    ...
    await page.evaluate(()=>{document.dispatchEvent(new CustomEvent('niakgpt:activity-changed',{detail:{state:'thinking',active:true,at:600}}));document.dispatchEvent(new CustomEvent('niakgpt:activity-changed',{detail:{state:'ready',active:false,at:700}}));});await page.waitForTimeout(320);
    const liveSignal=await page.evaluate(()=>{const c=window.__labStore['niakgpt-v08-cache'].chats.find(x=>x.id.startsWith('aaaaaaaa'));return{updated:c.updated,attentionAt:c.attentionAt||0};});
    assert(liveSignal.updated===200&&liveSignal.attentionAt===700,'response completion must publish attentionAt without forging server updated');

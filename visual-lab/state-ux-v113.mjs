...
for(const [engine,launcher] of Object.entries(engines)){
  ...
  try{
    await page.addInitScript(()=>{
      ...
      const store={'niakgpt-v08-cache':{projects:[{id:'g-p-niakgpt',name:'NiakGPT'}],chats:[{id:id1,title:'Correct Server Title',projectId:'g-p-niakgpt',updated:200},{id:id2,title:'Background Chat',projectId:'g-p-niakgpt',updated:100}]}};const listeners=[];
      const pick=key=>typeof key==='string'?{[key]:store[key]}:Array.isArray(key)?Object.fromEntries(key.map(k=>[k,store[k]])):{...store};
      window.__labStore=store;window.__labNotify=(key,next)=>{const old=store[key];store[key]=next;for(const fn of listeners)fn({[key]:{oldValue:old,newValue:next}},'local');};
    ...
    await page.evaluate(()=>{const raw=structuredClone(window.__labStore['niakgpt-v08-cache']);raw.chats=raw.chats.map(c=>c.id.startsWith('aaaaaaaa')?{...c,title:'Wrong Old Cache',updated:200}:c);window.__labNotify('niakgpt-v08-cache',raw);});await page.waitForTimeout(500);
    ...
    await page.evaluate(()=>{const raw=structuredClone(window.__labStore['niakgpt-v08-cache']);raw.chats=raw.chats.map(c=>c.id.startsWith('bbbbbbbb')?{...c,title:'Background Chat',updated:300}:c);window.__labNotify('niakgpt-v08-cache',raw);});await page.waitForTimeout(350);
    ...
    await page.locator('a[data-chat^="bbbbbbbb"]').click({noWaitAfter:true});await page.waitForTimeout(120);attention=await page.evaluate(()=>({unread:document.querySelector('a[data-chat^="bbbbbbbb"]')?.dataset.ng113Unread||''}));assert(attention.unread!=='1','opening chat did not clear unread state');
    await page.evaluate(()=>{document.documentElement.dataset.ng112LongThread='1';document.documentElement.dataset.ng8Heavy='1';document.querySelector('main article')?.remove();document.dispatchEvent(new CustomEvent('niakgpt:activity-changed',{detail:{active:false}}));});await page.waitForTimeout(500);
    ...
    const dir=path.join(OUT,engine);await fs.mkdir(dir,{recursive:true});await page.screenshot({path:path.join(dir,'state-ux.png'),fullPage:true});await fs.writeFile(path.join(dir,'state-ux.html'),await page.content());await fs.writeFile(path.join(dir,'state-ux.json'),JSON.stringify({crumb,canonical,attention,guard},null,2));

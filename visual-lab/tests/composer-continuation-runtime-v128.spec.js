...
test('real MV3 static continuation layer prefixes only pre-existing parallel work',async()=>{
  ...
  const context=await chromium.launchPersistentContext(dir,{headless:true,args:[`--disable-extensions-except=${extensionPath}`,`--load-extension=${extensionPath}`,'--disable-background-mode','--no-first-run','--no-default-browser-check']});
  ...
  try{
    ...
    await page.evaluate(()=>document.documentElement.dataset.ng86Activity='ready');
    ...
    await page.evaluate(()=>document.documentElement.dataset.ng86Activity='thinking');
    ...
    await page.evaluate(()=>document.documentElement.dataset.ng86Activity='executing');

...
test('real MV3 continuity pending survives full document navigation',async()=>{
  ...
  try{
    ...
    await expect(page.locator('#ng8-pins a[data-ng8-pin="1"]')).toHaveCount(1,{timeout:20000});
    ...
    const afterPage=await page.evaluate(key=>({session:sessionStorage.getItem(key),text:document.querySelector('#prompt-textarea')?.value||'',bootErrors:sessionStorage.getItem('niakgpt-last-boot-errors-v100')}),SESSION);
    ...
    await expect(page.locator('#prompt-textarea')).toContainText('CONTINUITÉ NIAKGPT',{timeout:10000});
  }finally{

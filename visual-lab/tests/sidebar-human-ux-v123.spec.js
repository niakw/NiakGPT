...
test(`full human sidebar session UX (${BROWSER_LABEL}, ${PROFILE_NAME})`,async()=>{
  ...
  try{
    ...
    await test.step('Project and chat action hitboxes are reliable, exclusive and true toggles',async()=>{
      const page=rt.page;await page.evaluate(()=>{const list=document.querySelector('#ng8-pins>.ng8-pin-list');list.dispatchEvent(new WheelEvent('wheel',{deltaY:-240,bubbles:true,cancelable:true}));list.scrollTop=0;});await page.waitForTimeout(120);await expect(projectAction(page)).toHaveCount(1,{timeout:5000});await projectAction(page).scrollIntoViewIfNeeded();
      const geometry=await page.evaluate(({P1})=>{const entry=document.querySelector(`#ng8-pins .ng96-pin-entry[data-pid="${P1}"]`),a=entry.querySelector(':scope>a'),b=entry.querySelector(':scope>.ng113-native-actions-project'),ar=a.getBoundingClientRect(),br=b.getBoundingClientRect(),hit=(r)=>document.elementFromPoint((r.left+r.right)/2,(r.top+r.bottom)/2);return{gap:br.left-ar.right,a:!!hit(ar)?.closest('a[data-ng8-pin]'),ab:!!hit(ar)?.closest('.ng113-native-actions'),b:!!hit(br)?.closest('.ng113-native-actions-project'),ba:!!hit(br)?.closest('a[data-ng8-pin]'),bw:br.width,bh:br.height,aw:ar.width,ah:ar.height,entryW:entry.getBoundingClientRect().width};},{P1});expect(geometry.gap).toBeGreaterThanOrEqual(0);expect(geometry).toMatchObject({a:true,ab:false,b:true,ba:false});expect(geometry.bw).toBeGreaterThanOrEqual(28);expect(geometry.bh).toBeGreaterThanOrEqual(28);expect(geometry.aw).toBeGreaterThan(80);expect(geometry.ah).toBeGreaterThanOrEqual(34);expect(geometry.entryW).toBeGreaterThan(geometry.aw);
    ...
    await test.step('Project custom rename targets only the exact Project native row',async()=>{
      const page=rt.page;await page.evaluate(()=>document.querySelector('#ng8-pins>.ng8-pin-list').scrollTop=0);await projectAction(page).click();await page.getByRole('menuitem',{name:'Renommer…'}).click();await expect(page.locator('#ng123-rename-dialog')).toHaveCount(1);await page.locator('#ng123-rename-dialog input').fill('Studio Renommé UX');await page.locator('#ng123-rename-dialog [data-save]').click();try{await expect(page.locator('#ng123-rename-dialog')).toHaveCount(0,{timeout:7000});}catch(error){const diag=await page.evaluate(()=>({stage:document.documentElement.dataset.ng123ProjectRename||'',status:document.querySelector('#ng123-rename-dialog small')?.textContent||'',nativeMenus:[...document.querySelectorAll('[role="menu"],[data-radix-menu-content]')].filter(m=>!m.closest('#ng123-action-menu')).map(m=>({text:(m.textContent||'').trim(),display:getComputedStyle(m).display,visibility:getComputedStyle(m).visibility})).slice(0,5),nativeDialogs:[...document.querySelectorAll('[role="dialog"]')].filter(d=>d.id!=='ng123-rename-dialog').map(d=>({id:d.id,text:(d.textContent||'').trim(),display:getComputedStyle(d).display,visibility:getComputedStyle(d).visibility})).slice(0,5),nativeProjectMarks:[...document.querySelectorAll('[data-ng112-native-projects="1"]')].map(n=>({tag:n.tagName,cls:n.className,id:n.id,display:getComputedStyle(n).display,visibility:getComputedStyle(n).visibility})).slice(0,8)}));throw new Error(error.message+'\nPROJECT_RENAME_DIAG '+JSON.stringify(diag));}await expect(studioPin(page)).toContainText('Studio Renommé UX',{timeout:10000});expect(rt.traffic.projectRenames).toBeGreaterThanOrEqual(1);expect(rt.traffic.patches.some(p=>p.body.title==='Studio Renommé UX')).toBe(false);
    });

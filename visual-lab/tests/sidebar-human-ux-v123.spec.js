...
test(`full human sidebar session UX (${BROWSER_LABEL}, ${PROFILE_NAME})`,async()=>{
  ...
  try{
    ...
    await test.step('Project and chat action hitboxes are reliable, exclusive and true toggles',async()=>{
      ...
      for(let i=0;i<8;i++){await projectAction(page).click();await expect(page.locator('#ng123-action-menu[data-kind="project"]')).toHaveCount(1);const g=await menuGeometry(page);expect(g).toMatchObject({body:true,position:'fixed',hit:true,inside:true});expect(g.left).toBeGreaterThanOrEqual(g.sideRight+2);expect(g.z).toBeGreaterThan(100000);const projectMenuText=await page.locator('#ng123-action-menu').innerText();expect(projectMenuText).toContain('Paramètres du projet');expect(projectMenuText).toContain('Actualiser les conversations');expect(projectMenuText).not.toContain('Déplacer vers');await projectAction(page).click();await expect(page.locator('#ng123-action-menu')).toHaveCount(0);}

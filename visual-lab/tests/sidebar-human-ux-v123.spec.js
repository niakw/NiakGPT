...
test(`full human sidebar session UX (${BROWSER_LABEL}, ${PROFILE_NAME})`,async()=>{
  ...
  try{
    ...
    await test.step('Project folder and chat drawer keep independent scroll positions',async()=>{
      const page=rt.page;await studioPin(page).click();await expect(drawer(page)).toHaveCount(1,{timeout:5000});await waitStudioChats(page);
      ...
      await studioPin(page).click();await expect(drawer(page)).toHaveCount(0);await studioPin(page).click();await waitStudioChats(page);const reopened=await scrollState(page);expect(Math.abs(reopened.inner-before.inner)).toBeLessThanOrEqual(5);
    });
    ...
    await test.step('Long-work soak: NiakGPT never aborts or navigates without a trusted incident',async()=>{
      const page=rt.page;await rt.goto(`/c/${CHAT1}`);const docs=rt.traffic.documents,posts=rt.traffic.posts,url=page.url();await page.evaluate(()=>{window.__humanAutoClicks=0;document.addEventListener('click',e=>{if(e.isTrusted)window.__humanAutoClicks++;},true);document.documentElement.dataset.ng86Activity='thinking';document.documentElement.dataset.ng8Running='1';document.dispatchEvent(new CustomEvent('niakgpt:activity-changed',{detail:{active:true,state:'thinking'}}));});
      for(let i=0;i<80;i++){await page.evaluate(i=>{const t=document.createElement('article');t.dataset.testid=`conversation-turn-soak-${i}`;const m=document.createElement('div');m.setAttribute('data-message-author-role','assistant');m.textContent=i%13===0?'Discussion normale : “network error” ou “conversation limit” peuvent être cités sans être une alerte.':`Travail complexe en cours ${i}`;t.appendChild(m);document.querySelector('.content').insertBefore(t,document.querySelector('.composer'));},i);await page.waitForTimeout(110);}
      expect(page.url()).toBe(url);expect(rt.traffic.documents).toBe(docs);expect(rt.traffic.posts).toBe(posts);await expect(page.locator('#ng119-interruption')).toHaveCount(0);expect(await page.evaluate(()=>window.__humanAutoClicks)).toBe(0);await page.evaluate(()=>{document.documentElement.dataset.ng86Activity='ready';document.documentElement.dataset.ng8Running='0';document.dispatchEvent(new CustomEvent('niakgpt:activity-changed',{detail:{active:false,state:'ready'}}));});
    });

...
test(`real extension target Studio (${BROWSER_LABEL}, ${PROFILE_NAME})`,async()=>{
  let rt;
  ...
  try{
    await test.step('native Projects hidden',async()=>{await expect.poll(()=>nativeProjectsVisible(rt.page),{timeout:7000}).toEqual([]);});
    await test.step('human targets Studio by stable Project id, not transient visual order',async()=>{
      await expect(studioPin(rt.page)).toHaveCount(1,{timeout:10000});
      await studioPin(rt.page).click();
      await expect(rt.page.locator(`#ng8-pins .ng96-pin-drawer[data-pid="${P1}"]`)).toHaveCount(1,{timeout:5000});
      await expect.poll(async()=>{const s=await drawerSnapshot(rt.page);return s.pid===P1&&s.rows>0&&s.actions===s.rows&&s.rowsWithOneAction===s.rows&&s.duplicateActions===0;},{timeout:5000}).toBe(true);
    });
    await test.step('cold index converges Studio to its exact two backend chats',async()=>{
      try{await expect.poll(async()=>{const s=await drawerSnapshot(rt.page);return{pid:s.pid,ids:[...s.chatIds].sort(),actions:s.actions,dup:s.duplicateActions};},{timeout:18000}).toEqual({pid:P1,ids:[CHAT1,CHAT2].sort(),actions:2,dup:0});}
    ...
    await test.step('unrelated ChatGPT churn preserves drawer and actions',async()=>{

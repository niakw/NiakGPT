...
async function drawerSnapshot(page){
  return page.evaluate(()=>{
    ...
    return {rows:rows.length,actions:actions.length,uniqueRows:new Set(rows.map(r=>r.dataset.chat)).size,rowsWithOneAction:rows.filter(r=>r.querySelectorAll(':scope>.ng113-native-actions-chat').length===1).length,duplicateActions:rows.filter(r=>r.querySelectorAll(':scope>.ng113-native-actions-chat').length>1).length};
  });
}
...
test(`real extension (${BROWSER_LABEL}): Projects hidden, progressive actions complete, sidebar stable`,async()=>{
  ...
  await test.step('boot exact browser + real MV3 extension',async()=>{
    ...
    if(BROWSER_LABEL.includes('brave')){
      const brave=await rt.page.evaluate(()=>({hasBrave:!!navigator.brave,ua:navigator.userAgent}));
      expect(brave.hasBrave,`Expected Brave runtime, got ${brave.ua}`).toBe(true);
      console.log(`Brave runtime confirmed: ${brave.ua}`);
    }
  });
  try{
    await test.step('native Projects hidden, recents untouched',async()=>{
    ...
    await test.step('human opens pin; every visible chat gets exactly one action',async()=>{
      await rt.page.locator('#ng8-pins a[data-ng8-pin="1"]').first().click();
    ...
    await test.step('progressive backend index converges to all chats + actions',async()=>{
    ...
    await test.step('unrelated ChatGPT DOM/class churn does not rebuild drawer or blink actions',async()=>{
      await rt.page.evaluate(()=>{
        ...
        const pins=document.getElementById('ng8-pins');const action=pins.querySelector('.ng113-native-actions-chat');action.dataset.realRuntimeToken='keep';action.focus();window.__focusedAction=action;
        window.__sidebarObs=new MutationObserver(rs=>{for(const r of rs)for(const n of r.removedNodes){if(!(n instanceof Element))continue;window.__sidebarChurn.removedActions+=n.matches('.ng113-native-actions-chat')?1:n.querySelectorAll?.('.ng113-native-actions-chat').length||0;window.__sidebarChurn.removedDrawers+=n.matches('.ng96-pin-drawer')?1:n.querySelectorAll?.('.ng96-pin-drawer').length||0;}});window.__sidebarObs.observe(pins,{childList:true,subtree:true});
    ...
    await test.step('React-like text-only Projects remount is hidden again',async()=>{
      await rt.page.evaluate(()=>{
        const nav=document.querySelector('aside[data-testid="conversation-sidebar"] nav');nav.querySelector('.project-list')?.remove();[...nav.querySelectorAll('h3')].find(x=>/projects?/i.test(x.textContent||''))?.remove();
        const h=document.createElement('div');h.textContent='Projets';h.id='runtime-remount-title';const list=document.createElement('div');list.id='runtime-remount-projects';list.innerHTML='<button role="button">Studio</button><button role="button">Research Lab</button>';nav.insertBefore(h,nav.querySelector('.recent-list'));nav.insertBefore(list,nav.querySelector('.recent-list'));
      });
    ...
    await test.step('zero extension JS/console errors',async()=>{
      expect(rt.pageErrors).toEqual([]);expect(rt.consoleErrors).toEqual([]);await rt.page.evaluate(()=>window.__sidebarObs?.disconnect());
    });

...
async function launchRuntime(){
  ...
  const launch={
    headless:HEADLESS,
    viewport:browserPrefs.viewport||{width:1440,height:900},
    colorScheme:browserPrefs.colorScheme||'dark',
    reducedMotion:browserPrefs.reducedMotion||'reduce',
...
test(`real extension (${BROWSER_LABEL}, ${PROFILE_NAME}): Projects hidden, progressive actions complete, sidebar stable`,async()=>{
  ...
  await test.step('boot exact browser + real MV3 extension + saved profile',async()=>{
    ...
    console.log(`${BROWSER_LABEL}/${PROFILE_NAME} runtime: ${JSON.stringify(state)}`);

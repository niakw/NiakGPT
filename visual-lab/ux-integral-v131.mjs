...
<aside id="ng8-rail" aria-label="Outils NiakGPT"><button aria-label="Explorer">▤</button><button aria-label="Sommaire">☷</button><button data-q aria-label="Quick Open">⌘</button></aside><aside id="ng8-panel"></aside><div id="ng8-status"><span class="ng8-version">NiakGPT 0.9.77</span><span class="ng8-status-project">Alpha</span><button data-q>⌘ Alt+K</button><strong>BY SKYNET</strong><span class="ng8-core-state">PRÊT</span></div>
<script>window.__store={'niakgpt-v08-cache':{projects:[{id:'g-p-alpha',name:'Alpha'},{id:'g-p-beta',name:'Beta'}],chats:[]}};window.chrome={storage:{local:{get:async key=>{const keys=Array.isArray(key)?key:[key];const out={};for(const k of keys)out[k]=window.__store[k];return out;}},onChanged:{addListener:()=>{}}},runtime:{getManifest:()=>({version:'0.9.77'})}};</script>
</body></html>`;
try{
  ...
  const expanded=await page.evaluate(()=>({hidden:document.querySelector('#ng8-coach .ng131-coach-detail')?.hidden,preview:document.querySelector('#ng8-coach .ng100-prompt-preview')?.textContent||''}));assert(expanded.hidden===false,'coach did not expand on explicit request');assert(expanded.preview.includes('CRITÈRES DE RÉUSSITE'),'expanded coach lost optimization content');
